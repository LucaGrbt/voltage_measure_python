...
# import Main GUI + Calibration Window GUI
...
"""
Additional Arduino Sketch with standard potentiometer connected to analog input:
...
void loop() {
  ...
  float vol = sensorValue * 5/1023;
  // print out the value you read:
  Serial.println(vol, 4);
  delay(5);        // delay in between reads for stability
}

----> beware: delay of 5 ms (means about 200 Hz signal) is about the bottle neck of this code, maybe due to Pythons slow speed? pure C++ code might be the more favourable language here or a much more effective Python code (search for PyShine on Google)
"""
...
class Messprogramm(QMainWindow, Ui_MainWindow):  # put in Ui_MainWindow that was converted by uic to setupUi later
    def __init__(self, parent=None):
        ...
        self.start_measure.clicked.connect(
            lambda: self.start_functions())  # difficutlies appeared when connecting two functions to on button directly, so there was one function connected to the button which later calls both wanted functions
    ...
    def append_calibration_data(self):
        self.calibration_data.append([float(self.calibration_ui.current_weight_input.text()),
                                      float(self.calibration_ui.current_measure_display.text())])
    ...
    def save_calibration_data(self):
        if self.students_data.text() == "Hier Gruppe/Namen eingeben zum Speichern der Daten" or "/" in self.students_data.text() or ":" in self.students_data.text():
            ...
            file = xlsxwriter.Workbook(
                "/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-EICHKURVE-" + str(
                    self.students_data.text()) + "-" + str(self.get_datetime()) + ".xlsx")
            ...
            table.write(0, 0,
                        "Jenike_Messdaten-EICHKURVE-" + str(self.students_data.text()) + "-" + str(self.get_datetime()))
            ...
            plt.axis([0, 6, 0, 2])
            plt.grid(True)
            plt.savefig("/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-EICHKURVE-Plot-" + str(
                self.students_data.text()) + "-" + str(self.get_datetime()) + ".pdf")
            plt.show()
    ...
    def update_voltage_plot(self, ser_data, process_time):
        ...
        if process_time >= self.voltage_plot.minute_counter * 60:
            ...
            self.voltage_plot.xticks[0][1] = (
            self.voltage_plot.minute_counter * 60, str(self.voltage_plot.minute_counter * 60))
    ...
    def save_xlsx(self):
        if self.students_data.text() == "Hier Gruppe/Namen eingeben zum Speichern der Daten" or "/" in self.students_data.text() or ":" in self.students_data.text():
            ...
            data_voltage = self.voltage_plot.save_voltage
            file = xlsxwriter.Workbook(
                "/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-SCHERZELLE-" + str(
                    self.students_data.text()) + "-" + str(self.get_datetime()) + ".xlsx")
            table = file.add_worksheet()
            # table.writer(0, 0, "Versuch Jenike Scherzelle"+str(lineEdit Eingabefeld)+str(Datum))
            table.write(0, 0, "Jenike_Messdaten-SCHERZELLE-" + str(self.students_data.text()) + "-" + str(
                self.get_datetime()))
            ...
            # plt.plot(x, y, 'b,') # plot single dots
            plt.plot(x, y)  # plot continuous line
            plt.xlabel("Zeit [s]")
            plt.ylabel("Spannung [V]")
            plt.title("Spannung Jenike Scherzelle_" + self.get_datetime())
            # plt.axis([0, 180, 0, 5])
            plt.grid(True)
            plt.savefig("/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-SCHERZELLE-Plot-" + str(self.students_data.text()) + "-" + str(self.get_datetime()) + ".pdf")
            plt.show()
...
class ThreadClassPlot(QThread):
    ...
    def run(self):  # run(self) function will be called as the thread is started by button
        ...
        while (True):
            ...
            mt.sleep(0.001)
            """
            # ATTENTION! BOTTLENECK with Windows/Arduino combination: 
            # a code sampling rate of 1000 Hz (1 ms delay) works quite fine for a signal rate of 200 Hz (5 ms delay) 
            # Anything above 200 Hz from Arduino is barely possible with this code due to lack of efficiency 
            # use e.g. PyShine (PyQt based ready to use solution) or pure C++ code instead
            # PyQt's QTimer might also be a much better solution for the measurement timing, time()-Module is probably not made for this usecase, especially with multiple threads
            """
            self.process_time = mt.time() - process_time_pause + process_time_run

            # use "serial" module for arduino data
            ser_data = float(ser.readline())

            # use "gpiozero" module for raspberry pi data with MCP3008
            # voltage = adc.value
            # ser_data = voltage * 3.3 - 0.0239  # gpio emits signals from 0 (0V) to 1 (3.3V) - formula necessary to get correct voltage including other electric impacts from crowbar and RaspberryPi4 (Klemmschaltung/Ueberspannungsschutz)etc.
            # # ser_data = voltage * 0.4 - 0.0089                          # adjust V_ref here and for all other instances, STANDARD for "Raspio Analog Zero" is 3.3V from Raspberry
...
class ThreadClassProgress(QThread):
    ...
    def run(self):  # run(self) function will be called as the thread is started by button
        ...
        while (True):
            ...
            self.shear_info.emit(
                self.process_time)  # progress bar only needs time, no matter if int, or float format (time naturally given in float)
...
class ThreadClassCalibration(QThread):
    ...
    def run(self):  # run(self) function will be called as the thread is started by button
        while (True):
            ct.sleep(0.001)  # manipulate measurement frequency
...
if __name__ == '__main__':
    ...
    ser = serial.Serial('COM3', 115200)                                      # serial for Arduino data reading
    # adc = MCP3008(channel=0)  # gpio for Raspberry Pi data reading with MCP3008
