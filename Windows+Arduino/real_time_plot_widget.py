...
# from serial import Serial # use serial for Arduino usage
# from gpiozero import MCP3008 # use gpiozero for Rasperry Pi usage
# import csv
import queue
...
class VoltagePlot(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        #  Set up plot
        self.analogPlot = self.canvas.addPlot(title="Spannungsverlauf Output Scherzelle")
        ...
        self.x_axis.setLabel(text='Zeit [s]')  # set axis labels
        self.y_axis.setLabel(text='Output Messbruecke [V]')

        self.minute_counter = 0
        self.xticks = [[(0, '0'), (60, '60')],
                       []]  # x ticks are automatically extended by main_Messprogramm, update_voltage_plot function
        self.yticks = [[(0, '0'), (1, '1')],
    ...
    def _update(self):
        if self.plot_data.empty():
            ...
            if self.x.size >= self.numPoints:  # make sure that the size of the array includes the most recent data points
                self.x = np.append(self.x[0:self.numPoints:2],
                                   xAxis)  # plot whole process time, while lowering resolution with growing process time
