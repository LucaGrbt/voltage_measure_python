...
#if not already done: activate SPI interface of raspberry pi
    #raspberry terminal: raspi-config and look for SPI enabling
...
# import other necessary modules
    #time module has to be imported for every thread or other applications, otherwise there will be overlaps in module usage that freeze the program
import time
...
import time as ht   # import module for trigger holding time
import numpy as np
...
class Messprogramm(QMainWindow, Ui_MainWindow):                         # put in Ui_MainWindow that was converted by uic to setupUi later
    ...
    def save_calibration_data(self):
        if self.students_data.text() == "Hier Gruppe/Namen eingeben zum Speichern der Daten" or "/" in self.students_data.text() or ":" in self.students_data.text():
            ...
            file = xlsxwriter.Workbook("/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-EICHKURVE-"+str(self.students_data.text())+"-"+str(self.get_datetime())+".xlsx")
            ...
            plt.axis([0, 10, 0, 2])
            plt.grid(True)
            plt.savefig("/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-EICHKURVE-Plot-"+str(self.students_data.text())+"-"+str(self.get_datetime())+".pdf")
            plt.show()
    ...
    def switch_testrig_on(self):
        ...
        elif ht.time()-self.trigger_hold >= 1: 
    ...
    def save_xlsx(self):
        if self.students_data.text() == "Hier Gruppe/Namen eingeben zum Speichern der Daten" or "/" in self.students_data.text() or ":" in self.students_data.text():
            ...
            data_time = self.voltage_plot.save_time
            data_voltage = self.voltage_plot.save_voltage
            file = xlsxwriter.Workbook("/home/mvt/Desktop/SA_MVT_Jenike/MessDaten_Plots/Jenike_Messdaten-SCHERZELLE-"+str(self.students_data.text())+"-"+str(self.get_datetime())+".xlsx")
            table = file.add_worksheet()
            ...
            plt.plot(x, y, linewidth=1) # plot continuous line
            plt.xlabel("Zeit [s]")
            plt.ylabel("Spannung [V]")
...
class ThreadClassPlot(QThread):
    ...
    def run(self):                                                      # run(self) function will be called as the thread is started by button
        ...
        while (True):
            ...
            voltage = adc.value
            ser_data = voltage * 3.3                                    # gpio emits signals from 0 (0V) to 1 (3.3V) - formula necessary to get correct voltage including other electric impacts from crowbar and RaspberryPi4 (Klemmschaltung/Ueberspannungsschutz)etc.
...
class ThreadClassCalibration(QThread):
    ...
    def run(self):                                                      # run(self) function will be called as the thread is started by button
        while (True):
            ...
            voltage = adc.value
            ser_data = voltage * 3.3                                    # gpio emits signals from 0 (0V) to 1 (3.3V) - formula necessary to get correct voltage including other electric impacts from crowbar (Klemmschaltung/Ueberspannungsschutz)etc.
            #ser_data = voltage * 3.3 - 0.0239                          # mit offset
...
if __name__ == '__main__':        
    ...
    # ser = serial.Serial('/dev/ttyACM0', 115200)                                      # serial for Arduino data reading
    adc = MCP3008(channel=7)                                  # gpio for Raspberry Pi data reading with MCP3008
